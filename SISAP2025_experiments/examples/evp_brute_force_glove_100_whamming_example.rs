...
fn main() -> Result<()> {
    ...
    let data_bitreps = f32_data_to_evp::<1>(data,200);                      // 200 bits selected
    let queries_bitreps = f32_data_to_evp::<1>(queries,200);
    ...
    let hamming_distances = generate_hamming_dists::<1>(queries_bitreps,data_bitreps);
    let after = Instant::now();
