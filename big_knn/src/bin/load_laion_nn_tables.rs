...
pub fn main() -> anyhow::Result<()> {
    ...
    println!("Neighbours shape: {:?}", neighbours.shape());
    println!("Sims shape: {:?}", sims.shape());

    println!(
        "Neighbours first row: {:?}",
        neighbours.row(0).slice(s![0..10])
    );
    println!("Sims first row: {:?}", sims.row(0).slice(s![0..10]));
    Ok(())
}
