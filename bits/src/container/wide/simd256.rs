...
pub type Simd256 = u64x4;

impl BitsContainer for Simd256 {
    fn new() -> Self {
        Self::default()
    }
...
#[cfg(test)]
mod tests {
    use crate::container::{BitsContainer, Simd256};

    #[test]
    fn bits_default() {
        let bits = <Simd256>::default();

        assert_eq!(&bits.into_u64_iter().collect::<Vec<_>>(), &[0; 4]);
    }

    #[test]
    fn bits_set_zero() {
        let mut bits = <Simd256>::default();

        bits.set_bit(0, true);
        assert_eq!(&bits.into_u64_iter().collect::<Vec<_>>(), &[1, 0, 0, 0,]);

        bits.set_bit(0, false);
        assert_eq!(bits, <Simd256>::default());
    }
    ...
    fn bits_65() {
        ...
        assert_eq!(&bits.into_u64_iter().collect::<Vec<_>>(), &[0, 2, 0, 0,]);
    }
}
