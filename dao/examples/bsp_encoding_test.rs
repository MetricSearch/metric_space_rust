...
fn main() -> Result<()> {
    ...
    println!(
        ...
        data_0.ones.count_ones() + data_0.negative_ones.count_ones(),
        data_0.ones.xor_cloned(&data_0.negative_ones).count_ones()
    );
    println!(
        "Data_1 has {} bits | XORed = {} bits",
        data_1.ones.count_ones() + data_1.negative_ones.count_ones(),
        data_1.ones.xor_cloned(&data_1.negative_ones).count_ones()
    );

    println!(
        "Object 0 bitrep ones: {:?} | negative ones: {:?}",
        data_0.ones, data_0.negative_ones
    );

    println!(
        "Smoking 0-0 distance {} similarity: {} ",
        bsp_distance::<2>(&data_0, &data_0),
        bsp_similarity::<2>(&data_0, &data_0)
    ); // two girls  --> 1024 + 200
    println!(
        "data 1-1 leaves distance {} similarity: {} ",
        bsp_distance::<2>(data_1, &data_1),
        bsp_similarity::<2>(data_1, &data_1)
    ); // smoking girl and leaves.
    println!(
        "data 2-2 leaves distance {} similarity: {} ",
        bsp_distance::<2>(data_2, &data_2),
        bsp_similarity::<2>(data_2, &data_2)
    ); // smoking girl and leaves.
    println!(
        "Smoking0-585 distance {} similarity: {} ",
        bsp_distance::<2>(data_585585, &data_0),
        bsp_similarity::<2>(data_585585, &data_0)
    ); // two girls
    println!(
        "Smoking 0-1 leaves distance {} similarity: {} ",
        bsp_distance::<2>(data_1, &data_0),
        bsp_similarity::<2>(data_1, &data_0)
    ); // smoking girl and leaves.
    println!(
        "Query 0-1 data distance {} similarity: {}  ",
        bsp_distance::<2>(query_0, &data_1),
        bsp_similarity::<2>(query_0, &data_1)
    ); // badness from other
