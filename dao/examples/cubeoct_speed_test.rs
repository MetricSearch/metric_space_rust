use anyhow::Result;
use bits::{f32_data_to_cubeoct_bitrep, whamming_distance};
...
fn main() -> Result<()> {
    ...
    let data_bitreps = f32_data_to_cubeoct_bitrep(data);
    let queries_bitreps = f32_data_to_cubeoct_bitrep(queries);
    ...
    println!(
        "Time per query 1_000_000 dists: {} ns",
