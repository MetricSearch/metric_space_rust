...
pub fn hdf5_pubmed_gt_load(
    ...
    let num_cols = if num_knns_required == 0 {
        num_cols
    } else {
    ...
    Ok((
        knns.slice(s![0..num_rows, 0..num_cols]).to_owned(),
...
pub fn filter_knns(knns: Array2<usize>, num_cols: usize) -> anyhow::Result<Array2<usize>> {
    ...
    for row in knns.rows() {
        let filtered: Vec<usize> = row.iter().take(num_cols).cloned().collect();

        filtered_rows.push(Array1::from(filtered));
    }

    // Stack all rows vertically to form an Array2
    let required_knns = ndarray::stack(
        Axis(0),
        &filtered_rows.iter().map(|x| x.view()).collect::<Vec<_>>(),
    )?;
