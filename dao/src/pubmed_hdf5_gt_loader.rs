...
pub fn hdf5_pubmed_gt_load(
    ...
    // let i_queries_dists = i_queries_group.dataset("dists").unwrap();
    let num_rows = i_queries_knns.shape()[0] as usize;
    let num_cols = i_queries_knns.shape()[1] as usize;

    let num_cols = if num_knns_required == 0 { num_cols } else { num_knns_required.min(num_cols) } as usize;

    let knns: Array2<usize> = i_queries_knns.read_slice(s![0..num_rows, 0..num_cols]).unwrap();
    let dists: Array2<f32> = i_queries_dists.read_slice(s![0..num_rows, 0..num_cols]).unwrap();
