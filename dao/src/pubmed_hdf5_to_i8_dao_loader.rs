...
pub fn hdf5_pubmed_f32_to_i8_load(
    ...
    for start in (0..num_records).step_by(rows_at_a_time) {
        ...
        let i_8_array2: Array2<i8> = stack(
            Axis(0),
            &i_8_rows_vec.iter().map(|r| r.view()).collect::<Vec<_>>(),
        )
        .unwrap();

        i8_data.slice_mut(s![start..end, 0..]).assign(&i_8_array2);
    }
    ...
    let i_8_i_test_array: Array2<i8> = stack(
        Axis(0),
        &i_8_i_test_vec.iter().map(|r| r.view()).collect::<Vec<_>>(),
    )
    .unwrap();

    let all_combined: Array2<i8> =
        concatenate(Axis(0), &[i8_data.view(), i_8_i_test_array.view()]).unwrap();
