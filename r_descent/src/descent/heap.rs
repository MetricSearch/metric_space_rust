...
pub struct Heap {
    ...
    pub distances : Vec<Vec<NonNan>>, // ArrayBase<OwnedRepr<NonNan>, Dim<[usize; 2]>>,
...
impl Heap {
    pub fn new(num_entries: usize, num_neighbours: usize) -> Heap {
        ...
        let distances =  vec![vec![NonNan::new(f32::MAX).unwrap(); num_neighbours]; num_entries];  // Array::from_elem((num_entries, num_neighbours), NonNan::new(f32::MAX).unwrap());
