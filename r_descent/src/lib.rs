            ...
            while !finished {
                if candidates_list.len() == 0 {
                        ...
                        && nearest_candidate_pair.distance > results_list.peek().unwrap().distance
                    {
                        ...
                        if results_list.len() > num_neighbours {   // was ef <<<<<<
...
pub fn initialise_table_m(dao: Rc<DaoMatrix<f32>>, chunk_size: usize, num_neighbours: usize) -> (Array2<usize>, Array2<f32>) {
    ...
    result_indices
        ...
        .for_each(|(i, (mut result_indices_chunk, mut result_sims_chunk))| {
            ...
            let mut closest_dao_indices: Array2<usize> = Array2::<usize>::zeros((real_chunk_size, num_neighbours));
   ...
   (insert_index_at_position_1_inplace(result_indices), insert_column_inplace(result_sims,1.0))
}

pub fn get_nn_table2_m(dao: Rc<DaoMatrix<f32>>,
...
pub fn get_reverse_links_not_in_forward(neighbours: &Array2<usize>, similarities: &Array2<f32>, reverse_list_size: usize) -> (Array2<usize>, Array2<f32>) {
    ...
    for row in 0..num_data { // Matlab line 97
        ...
        // so for each one of these (there are k...):
        for col in 0..num_neighbours { // Matlab line 99 (updated)
            // get the id
            ...
            let neighbours_of_next_dont_contain_current_row = !neighbours_of_next_id_in_row.iter().any(|x| *x == row);

            println!("Row {} col {} next_id {} sim {} neighbours of next {} don't contain {} row {}", row, col, next_id_in_row, next_sim_in_row, neighbours_of_next_id_in_row, row, neighbours_of_next_dont_contain_current_row);
            ...
            if neighbours_of_next_dont_contain_current_row {
                ...
                if reverse_count[*next_id_in_row] < reverse_list_size { // if the list is not full
                    // update the reverse pointer list and the similarities
                    println!("Adding row {} refers to {} insert position {}", row, *next_id_in_row, reverse_count[*next_id_in_row] );
                    ...
                    reverse_count[*next_id_in_row] = reverse_count[*next_id_in_row] + 1; // increment the count
                } else {
                    ...
                    println!("full min index in {} and value of row {} are {} {}", &reverse_sims.row(*next_id_in_row), row, position, value);
                    if value < next_sim_in_row { // Matlab line 110  if the value in reverse_sims is less similar we over write
                        println!( "overwriting" );
...
mod tests {
    ...
    pub fn test_reverse_links() {
        ...
        let reverse_gt_2 : Vec<usize> = vec![
            4, 18446744073709551615,    // 3 & 4 refer to 0 - but 3 in forward links
            0, 18446744073709551615 ,   // 0 refers to 1
            0, 1,                       // 0, 1, 2, 3, 4 refer to 2 but 2,3,4 are in the forward => 0,1
            1, 18446744073709551615,    // 0,1,2,4 to 3 - but 0,2,4 in forward => 1
            1, 18446744073709551615, ]; // 1, 2, 3 refer to 4  - but 2,3 in forward -> 1
        ...
        let mut forward_links: Array2<usize> = Array2::from_shape_vec((5, 3), forward_links).unwrap();
        ...
        let reverse_links = get_reverse_links_not_in_forward(&&mut forward_links, &&mut forward_sims, 2);

        println!("Reverse links: {:?} reverse sims: {:?}", reverse_links.0, reverse_links.1);
...
pub fn initialise_table_bsp(dao: Rc<Dao<EVP_bits<2>>>, chunk_size: usize, num_neighbours: usize) -> (Array2<usize>, Array2<f32>) {
    ...
    if num_neighbours > chunk_size  {
        panic!("Error: num_neighbours {} > chunk_size {}", num_neighbours, chunk_size);
    }
    let mut result_indices = unsafe { Array2::<usize>::uninit((num_data, num_neighbours)).assume_init()};
    let mut result_sims = unsafe {Array2::<f32>::uninit((num_data, num_neighbours)).assume_init()};
    ...
    (insert_index_at_position_1_inplace(result_indices), insert_column_inplace(result_sims,1224.0))
...
pub fn get_nn_table2_bsp(dao: Rc<Dao<EVP_bits<2>>>,
    ...
    while work_done > ((num_data as f64) * delta) as usize { // Matlab line 61
        ...
        old
            ...
            .map(|((row, old_row), new_row)| {
                ...
                // index the data using the rows indicated in old_row
                let old_data = get_bsp_slice_using_selected(&data, &old_row.view(), [old_row.len()]);                         // Matlab line 136
                let new_data = get_bsp_slice_using_selected(&data, &new_row.view(), [new_row.len()]);                         // Matlab line 137
                let new_union_data = get_bsp_slice_using_selected(&data, &new_row_union.view(), [new_row_union_len]);         // Matlab line 137

                let new_new_sims: Array2<f32> = matrix_dot_bsp::<2>(&new_union_data.view(), &new_union_data.view(), |a, b| bsp_similarity_as_f32::<2>(a, b)); // <<<<<<<<< TODO CHECK!!
            ...
            .for_each(|(row, new_row, old_row, new_row_union, new_new_sims, new_data, old_data)| {
                ...
                let new_old_sims = matrix_dot_bsp::<2>(&new_data.view(), &old_data.view(), |a, b| bsp_similarity_as_f32::<2>(a, b));
    ...
    fn get_bsp_slice_using_selected(source: &ArrayView1<EVP_bits<2>>, selectors: &ArrayView1<usize>, result_shape: [usize; 1]) -> Array1<EVP_bits<2>> {
        ...
        for count in 0..selectors.len() { // was result_shape
            source.slice(s![selectors[count]]).assign_to(sliced.slice_mut(s![count]));
        }
