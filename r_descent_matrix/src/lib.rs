...
use utils::{arg_sort_big_to_small_matrix, arg_sort_small_to_big, index_of_min_a, index_of_min_v, min_index_and_value_a, min_index_and_value_v, minimum_in_a, minimum_in_v};
...
pub fn initialise_table_m(dao: Rc<DaoMatrix>, chunk_size: usize, num_neighbours: usize) -> (Array2<usize>,Array2<f32>) {
    ...
    for i in 0..num_loops {
        ...
        let (sorted_ords, sorted_dists)= arg_sort_big_to_small_matrix(&chunk_dists);
...
pub fn get_nn_table2(dao: Rc<DaoMatrix>,
    ...
    while work_done > (( num_data as f64 ) * delta ) as usize { // Matlab line 61
        ...
        for row in 0..num_data { // Matlab line 97
            ...
            for id in 0..num_neighbours { // Matlab line 99 (updated)
                ...
                if reverse_count[*this_id] < reverse_list_size { // if the list is not full
                    ...
                    let (position, value ) = min_index_and_value_a(&reverse_sims.row(*this_id)); // Matlab line 109
                ...
                for new_ind2 in new_ind1 + 1 .. new_row_union.len() { // Matlab line 147
                    ...
                    if this_sim > global_mins[u1_id] { // Matlab line 154
                        ...
                        if ! neighbours.row(u1_id).iter().any(|x| *x == u2_id) { // Matlab line 156
                            ...
                            let position = index_of_min_a(&similarities.row(u1_id)); // Matlab line 157
                            ...
                            global_mins[u1_id] = minimum_in_a(&similarities.row(u1_id));
                    ...
                    if global_mins[u2_id] < this_sim { // Matlab line 166
                        if ! neighbours.row(u2_id).iter().any(|x| *x == u1_id) {
                            let position = index_of_min_a(&similarities.row(u2_id));
                            ...
                            global_mins[u2_id] = minimum_in_a(&similarities.row(u2_id));
                            work_done = work_done + 1;
                        }
                    }
                } // Matlab line 175
                ...
                for new_ind2 in 0..old_row.len() {
                    ...
                    if this_sim > global_mins[u1_id] { // Matlab line 191
                        ...
                        if ! neighbours.row(u1_id).iter().any(|x| *x == u2_id) { // Matlab line 193
                            let position = index_of_min_a(&similarities.row(u1_id));
                            neighbours[[u1_id,position]] = u2_id;
                            similarities[[u1_id,position]] = this_sim;
                            //println!( "3 Updating similarities {} {} {} ", u1_id, wh, this_sim );
                            neighbour_is_new[[u1_id,position]] = true;
                            global_mins[u1_id] = minimum_in_a(&similarities.row(u1_id));  // Matlab line 198
                            work_done = work_done + 1;
                        }
                    }
        ...
        let overall_min = minimum_in_a( &global_mins.view() );
