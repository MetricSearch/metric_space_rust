use anyhow::Result;
use bits::{f32_data_to_hamming5bit, hamming_distance};
...
fn main() -> Result<()> {
    ...
    let data_bitreps = f32_data_to_hamming5bit::<15>(data, 200); // 200 bits selected
    let queries_bitreps = f32_data_to_hamming5bit::<15>(queries, 200);
