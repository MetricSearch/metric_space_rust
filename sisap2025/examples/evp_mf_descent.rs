...
use dao::Dao;
use dao::{convert_f32_to_bsp::f32_dao_to_bsp, csv_dao_loader::dao_from_csv_dir};
use descent::Descent;
...
fn main() -> Result<()> {
    ...
    let dao_f32 = Rc::new(dao_matrix_from_csv_dir(
    ...
    let dao_evp = f32_dao_to_bsp::<Simd256x4, 768>(dao_f32.clone(), 100);
    ...
    let queries_bitreps = f32_dao_to_bsp::<3>(queries, 100);

    let mut results: Vec<Vec<Pair>> = vec![];

    for q in queries_bitreps {
        let (_canidates_len, sorted_results) = descent.knn_search::<Simd256x4>>(
            q.clone(),
