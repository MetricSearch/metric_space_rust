use anyhow::Result;
use bits::{f32_data_to_evp, whamming_distance};
...
fn main() -> Result<()> {
    ...
    let data_bitreps = f32_data_to_evp::<6>(data, 200); // 200 bits selected
    let queries_bitreps = f32_data_to_evp::<6>(queries, 200);
    ...
    let hamming_distances = generate_hamming_dists::<6>(queries_bitreps, data_bitreps);
    let after = Instant::now();
...
fn report_queries(
    ...
    (0..num_queries).into_iter().for_each(|qi| {
        ...
        let hamming_set = hamming_nns.into_iter().map(|x| *x).collect::<HashSet<_>>();
        let gt_set = gt_nns.into_iter().map(|x| *x).collect::<HashSet<_>>();
